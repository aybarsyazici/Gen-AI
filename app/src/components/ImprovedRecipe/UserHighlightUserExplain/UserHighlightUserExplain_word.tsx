...
interface ClickableWordProps {
    ...
    handleDecline: (currentExplanation: string, index: number) => void,
    showPopover: boolean;
...
const ClickableWord: React.FC<ClickableWordProps> = React.memo(({
    ...
    console.log('word rerender: ', currentWordIndex, "& Got Refs: ", likeButtonRef, dislikeButtonRef, popRef, spanRef);
    return (
        <Popover
            content={
                <div>
                    <TextArea
                        ...
                        onChange={(e) => setWordExplanation((prevWordExplanation) => {
                    ...
                    <div className="like-dislike-container">
                        <Button className="like-button" onClick={() => handleAccept(wordExplanation, currentWordIndex)} ref={likeButtonRef}>
                            <LikeOutlined />
                        </Button>
                        <Button className="dislike-button" onClick={() => handleDecline(wordExplanation, currentWordIndex)}
...
export const ImprovedRecipeDisplayWordScale: React.FC<ImprovedRecipeDisplayProps> = ({ improvedRecipe, 
    ...
    const [wordExplanations, setWordExplanation] = useState<Map<number, string>>(new Map());
    ...
    const createTour = () => {
        ...
        refs.push({
            ...
            content: 'You\'ll be asked if you like this change or not. You\'ll also be required to write a small explanation on why you think we implemented this change',
            target: refMap['first-word-pop'],
            onNext: () => {
                handleAccept('asdfghjh', 0)
            },
            preventClose: true,
        });
        refs.push({
            ...
            onNext: () => {
                handleDecline('asdfghjj', 2);
                handleAccept('asdfghjj', 9);
            },
            preventClose: true,
        });
        refs.push({
    ...
    const handleDecline = (currentExplanation: string, index: number) => {
        // Check whether the explanation is longer than 5 characters
        if (currentExplanation.length < 5 && !doTour) {
            return;
        }
